...
class Encoder(nn.Module):
    def __init__(self, node_num, dim_in, dim_out, cheb_k, embed_dim, num_layers, gcn_layers, time_layers, alpha, droprate, time_drop):
        ...
        self.dcrnn_cells.append(TGCRNCell(node_num, dim_in, dim_out, cheb_k, embed_dim, self.gcn_layers, self.alpha, self.droprate))
        for _ in range(1, num_layers):
            self.dcrnn_cells.append(TGCRNCell(node_num, dim_out, dim_out, cheb_k, embed_dim, self.gcn_layers, self.alpha, self.droprate))
...
class TGCRN(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = Encoder(args.num_nodes, args.input_dim, args.rnn_units, args.cheb_k,
                                args.embed_dim, args.num_layers, self.gcn_layer , self.time_layer, self.alpha, self.droprate, self.time_drop)
